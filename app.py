...
async def search():
    ...
    # Run Api
    data_0 = await get_movie_info(title=input_0, params=querystring)
    data_1 = await get_movie_info(title=input_1, params=querystring)
...
async def fight():
    ...
    try:
        list_0, list_1 = await asyncio.gather(get_all_actors_info(cast_0), get_all_actors_info(cast_1))
    except Exception as e:
    ...
    # session used to access data across different flask routes without using URL parameters (big files don't work well with URLs)
